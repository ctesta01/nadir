...
#' NA_standards
#'
#' Non-applicable standards are listed below with their tags changed from
#' `@srrstatsTODO` to `@srrstatsNA`, placed together in this block, along with
#' explanations for why each of these standards have been deemed not applicable.
#'
...
#'   against existing implementations (G5.4b).
#' @srrstatsNA {G5.4c} No stored/migrated published-paper outputs;
#'   comparison is against maintained implementations directly.
#' @srrstatsNA {G5.10, G5.11, G5.11a, G5.12} No extended tests: the full
...
#'   response, so missing responses in newdata are unaffected.
...
#'   propagate unmodified to the user.
#' @srrstatsNA {RE4.1} Our approach is that an unfitted model specification is
#'   what a learner is:
#'   learners are unevaluated (data, formula) -> closure functions, so a
#'   learners + formulas list is a complete unfitted specification.
#' @srrstatsNA {RE4.3, RE4.6, RE4.7} A weighted ensemble of heterogeneous
#'   learners has no coefficient confidence intervals, parameter
#'   variance-covariance matrix, or convergence statistics. Learner-level
#'   fits are user-inspectable via $fit_learners for learners exposing
#'   these.
