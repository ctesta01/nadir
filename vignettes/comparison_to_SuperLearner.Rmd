...
sl_output <- nadir::super_learner(
  ...
  regression_formulas = c(
    ...
    medv ~ s(crim, by=rad) + s(rm) + zn + chas + age + dis + rad + tax + ptratio + black + lstat,
...
true_Y <- sl_output$holdout_predictions$medv

library(dplyr)

cv_mse <- sl_output$holdout_predictions |>
  dplyr::select(-medv) |>
  dplyr::mutate(
...
sl_closure <- function(data) {
  nadir::super_learner(
    data = data,
    regression_formulas = c(
      rep(c(medv ~ .), 7), # first 7 formulas are to just regress medv on everything
      # formula for mgcv::gam
      medv ~ s(crim, by=rad) + s(rm) + zn + chas + age + dis + rad + tax + ptratio + black + lstat,
      # formula for lme4::lmer
      medv ~ (1|rad) + zn),
...
true_Y <- sl_ciafactbook$holdout_predictions$birth_rate
sl_ciafactbook$holdout_predictions$birth_rate <- NULL

# MSE
sl_ciafactbook$holdout_predictions |>
  dplyr::mutate(
    across(everything(), ~ (. - true_Y)^2)) |>
  dplyr::summarize(across(everything(), mean))
...
ciafactbook_cv_output <- cv_super_learner(data = ciafactbook, sl_closure = ciafactbook_sl_closure,
                yvar = 'birth_rate')
