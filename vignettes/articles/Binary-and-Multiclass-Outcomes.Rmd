...
For an example, we show using the Boston dataset and creating a binary outcome for a 
regression problem, and then train a `super_learner()` to predict this binary outcome.  
...
of binary data, we want to make sure the loss function used is the `determine_weights_for_binary_outcomes()` 
function provided by `{nadir}`. We can do this either by setting `outcome_type = 'binary'` or 
by passing the function directly to `super_learner()` as 
the argument `determine_super_learner_weights = determine_weights_for_binary_outcomes`. 
...
head(trained_binary_super_learner$predict(data))

# classification table
data.frame(
  truth = data$high_crim, 
  prediction = round(trained_binary_super_learner$predict(data))) |> 
...
data.frame(
  truth = data$high_crim, 
  predicted_pr_of_1 = trained_binary_super_learner$predict(data)) |> 
...
Above we covered binary classification — now we turn to classification 
...
Let's make a nice plot showing predicted probabilities from the 
...
#| fig-dpi: 300
#| fig-retina: 3
prob_adelie <- predict(sl_learned_model, { d <- df; d$species <- 'Adelie'; d} )
prob_chinstrap <- predict(sl_learned_model, { d <- df; d$species <- 'Chinstrap'; d} )
prob_gentoo <- predict(sl_learned_model, { d <- df; d$species <- 'Gentoo'; d} )

dfprob <- data.frame(
  i = 1:nrow(df),
  adelie = prob_adelie,
  chinstrap = prob_chinstrap,
  gentoo = prob_gentoo)

dfprob <- dfprob |> tidyr::pivot_longer(
  cols = c('adelie', 'chinstrap', 'gentoo'),
  names_to = 'species',
  values_to = 'prob'
  )

df$i <- 1:nrow(df)
df$species <- tolower(as.character(df$species))

ggplot(dfprob, aes(x = i, y = prob, fill = species)) + 
  geom_col() + 
  facet_grid(species~.) + 
  geom_point(
    data = df,
    mapping = aes(x = i, y = 1.025, fill = species),
    color = 'black',
    size = .25,
    shape = 20, 
    alpha = 0.8) + 
  ...
  ggtitle("super_learner() Estimated Probability of Each Class by Row ID",
          "Model ensemble consists of 3 Neural Networks and a Vector-GLM") + 
  xlab("Row ID") + 
  ylab("Estimated Species Membership Probability") + 
  labs(
    caption = "Actual species recorded for each row id are indicated in small black dots.")
