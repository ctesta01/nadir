...
pi_hat_of_L <- propensity_model$predict(df)
one_minus_pi_hat_of_L <- 1-pi_hat_of_L

# get outcome predictions
m_of_0_exposure_and_L <- # counterfactual model, everyone untreated
  outcome_model$predict({ df |> mutate(A_treatment = 0) })

m_of_1_exposure_and_L <- # counterfactual model, everyone treated
  outcome_model$predict({ df |> mutate(A_treatment = 1) })

m_of_A_and_L <- outcome_model$predict(df) # outcome model for treatment received
